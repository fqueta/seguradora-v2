...
class ContractEventController extends Controller
{
    ...
    public function recent(Request $request)
    {
        ...
        $cacheKey = "contract-events:recent:tenant={$tenantId}:limit={$limit}:supplier={$supplier}:from={$from}:to={$to}:status={$statusFilter}";

        $resolver = function () use ($limit, $supplier, $from, $to, $statusFilter) {
            ...
            if (!$supplier || $supplier === 'alloyal') {
                $alloyalQuery = DB::table('usermeta')
                ...
                if ($from && $to) {
                    try {
                        $start = new \Carbon\Carbon($from);
                        $end = (new \Carbon\Carbon($to))->endOfDay();
                        $alloyalQuery->whereBetween('created_at', [$start, $end]);
                    } catch (\Throwable $e) {}
                }

                $alloyalRows = $alloyalQuery->get();
                $alloyalUserIds = $alloyalRows->pluck('user_id')->filter()->unique()->values()->all();
                $alloyalUsers = [];
                if (!empty($alloyalUserIds)) {
                    $alloyalUsers = \App\Models\User::query()
                        ->whereIn('id', $alloyalUserIds)
                        ->get(['id', 'name'])
                        ->keyBy('id')
                        ->all();
                }
                ...
                foreach ($alloyalRows as $row) {
                    $user = $alloyalUsers[$row->user_id] ?? null;
                    $events->push([
        ...
        try {
            $payload = Cache::remember($cacheKey, 30, $resolver);
