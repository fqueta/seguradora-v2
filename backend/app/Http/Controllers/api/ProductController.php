...
class ProductController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Configurar campos adicionais no campo config (JSON)
        $config = [];
        ...
        if (isset($validated['availability'])) {
            $config['availability'] = $validated['availability'];
        }
        if (isset($validated['rating'])) {
            $config['rating'] = $validated['rating'];
        }
        if (isset($validated['reviews'])) {
            $config['reviews'] = $validated['reviews'];
        }
        if (isset($validated['terms']) && is_array($validated['terms'])) {
            $config['terms'] = $validated['terms'];
        }
    ...
    public function update(Request $request, string $id)
    {
        ...
        // Atualizar demais campos em config
        $config = $mappedData['config'] ?? ($productToUpdate->config ?? []);
        if (isset($validated['image'])) {
            $config['image'] = $validated['image'];
        }
        if (isset($validated['availability'])) {
