  ...
  async getAll(filters: AccountsFilter = {}): Promise<PaginatedResponse<AccountPayable>> {
    ...
    Object.entries(filters).forEach(([key, value]) => {
      if (value !== undefined && value !== null) {
        // Mapear nomes esperados pelo backend
        if (key === 'startDate') {
          params.append('due_date_from', value.toString());
        } else if (key === 'endDate') {
          params.append('due_date_to', value.toString());
        } else {
    ...
    if (response && Array.isArray(response.data)) {
      return response as PaginatedResponse<AccountPayable>;
    }
    ...
    return {
      data: Array.isArray(response?.items) ? response.items : (Array.isArray(response?.data) ? response.data : []),
      total: response?.total ?? response?.count ?? 0,
      page: response?.page ?? response?.current_page ?? 1,
      limit: response?.limit ?? response?.per_page ?? 10,
      totalPages: response?.total_pages ?? response?.last_page ?? 1,
    };
  },
