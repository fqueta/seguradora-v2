...
export default function ProductEdit() {
  ...
  useEffect(() => {
    // Re-sincroniza categoria quando opções são carregadas
    const cat = product?.category ? String(product.category) : '';
    const hasOptions = Array.isArray(categories) && categories.length > 0;
  ...
  const buildUpdatePayload = (data: UpdateProductInput) => ({
    name: data.name,
    description: data.description,
    category: data.category,
    salePrice: data.salePrice,
    ...
    form.handleSubmit(async (data: UpdateProductInput) => {
      ...
      try {
        ...
        toast({
          ...
          description: 'Alterações salvas. Você pode continuar editando.',
        });
      } catch (error) {
        toast({
          title: 'Erro ao atualizar produto',
