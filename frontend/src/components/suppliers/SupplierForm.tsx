...
import { CreateSupplierInput } from '@/types/suppliers';
...
interface SupplierFormProps {
  form: UseFormReturn<CreateSupplierInput>;
  isLoading?: boolean;
}

/**
 * Componente de formulário para criação e edição de fornecedores
 */
export function SupplierForm({ form, isLoading = false }: SupplierFormProps) {
  const { register, formState: { errors }, watch, setValue } = form;
  const tipoPessoa = watch('tipo_pessoa');
  const [showPassword, setShowPassword] = useState(false);
  const { fetchCep, loading: loadingCep } = useCep();

  // Apply visual mask to inputs
  const handleMaskChange = (e: React.ChangeEvent<HTMLInputElement>, maskFunc: (val: string) => string) => {
    const originalValue = e.target.value;
    const maskedValue = maskFunc(originalValue);
    
    // Update input display value
    e.target.value = maskedValue;
    
    // Trigger react-hook-form change
    form.setValue(e.target.name as any, maskedValue, { shouldValidate: true });
  };
  ...
  const handleCepBlur = async (e: React.FocusEvent<HTMLInputElement>) => {
    ...
    if (cep.replace(/\D/g, '').length === 8) {
      ...
      if (address) {
        setValue('config.endereco', address.endereco);
        setValue('config.bairro', address.bairro);
        setValue('config.cidade', address.cidade);
        setValue('config.uf', address.uf);
        // Focus number field after finding address
        document.getElementById('config.numero')?.focus();
      }
    }
  };
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <Input
                  id="cpf"
                  {...register('cpf')}
                  onChange={(e) => handleMaskChange(e, cpfApplyMask)}
                  disabled={isLoading}
                  maxLength={14}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <Input
                  id="cnpj"
                  {...register('cnpj')}
                  onChange={(e) => handleMaskChange(e, cnpjApplyMask)}
                  disabled={isLoading}
                  maxLength={18}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => {
                  if (typeof phoneApplyMask === 'function') {
            ...
            <div>
              ...
              <div className="relative">
                <Input
                  id="config.cep"
                  {...register('config.cep')}
                  onChange={(e) => handleMaskChange(e, cepApplyMask)}
