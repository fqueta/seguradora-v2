...
interface AccountsPayableTableProps {
  ...
  groupBy?: 'none' | 'category' | 'paymentMethod' | 'supplier';
  startDate?: string;
  endDate?: string;
  search?: string;
}
 ...
 */
export const AccountsPayableTable: React.FC<AccountsPayableTableProps> = ({ categories, groupBy = 'none', startDate, endDate, search }) => {
  ...
  // groupedData será definido após helpers para evitar ReferenceError
  ...
  const groupedData = React.useMemo(() => {
    if (!Array.isArray(accounts) || !accounts.length || groupBy === 'none') return null;
    const map = new Map<string, AccountPayable[]>();
    accounts.forEach(a => {
      let key = '';
      if (groupBy === 'category') {
        key = getCategoryName(a.category);
      } else if (groupBy === 'paymentMethod') {
        key = a.paymentMethod ? getPaymentMethodLabel(a.paymentMethod) : '-';
    ...
    return Array.from(map.entries()).map(([name, items]) => ({
      name,
      items,
      total: items.reduce((acc, it) => acc + (Number(it.amount) || 0), 0),
      count: items.length,
    }));
  }, [accounts, groupBy]);
                    ...
                    <>
                      <TableRow key={`group-${group.name}`} className="bg-muted/50">
                        <TableCell colSpan={8} className="font-semibold">
                          {group.name} — Total {formatCurrency(group.total)} • {group.count} itens
                        </TableCell>
                      </TableRow>
